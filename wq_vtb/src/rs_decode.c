...
static bool reed_solomon_find_syndromes(field_t field, polynomial_t msgpoly, field_logarithm_t **generator_root_exp,
    ...
    for (uint16_t i = 0; i < min_distance; i++) {
...
static uint16_t reed_solomon_find_error_locator(correct_reed_solomon *rs, size_t num_erasures) {
    ...
    uint16_t delay_length = 1;

    for (uint16_t i = rs->error_locator.order; i < rs->min_distance - num_erasures; i++) {
        discrepancy = rs->syndromes[i];
        for (uint16_t j = 1; j <= numerrors; j++) {
        ...
        if (2 * numerrors <= i) {
            ...
            uint16_t temp_order = rs->error_locator.order;
...
// Chien search
bool reed_solomon_factorize_error_locator(field_t field, uint16_t num_skip, polynomial_t locator_log, field_element_t *roots,
...
void reed_solomon_find_error_values(correct_reed_solomon *rs) {
    ...
    // calculate each e(j)
    for (uint16_t i = 0; i < rs->error_locator.order; i++) {
...
void reed_solomon_find_error_locations(field_t field, field_logarithm_t generator_root_gap,
                                       ...
                                       uint16_t num_errors, uint16_t num_skip) {
...
static void reed_solomon_find_error_roots_from_locations(field_t field, field_logarithm_t generator_root_gap,
                                                         ...
                                                         field_element_t *error_roots, uint16_t num_errors) {
    for (uint16_t i = 0; i < num_errors; i++) {
...
static polynomial_t reed_solomon_find_error_locator_from_roots(field_t field, uint16_t num_errors,
...
void correct_reed_solomon_decoder_create(correct_reed_solomon *rs) {
    ...
    rs->syndromes = malloc( rs->min_distance * sizeof(field_element_t));
    rs->modified_syndromes = malloc(2 * rs->min_distance * sizeof(field_element_t));
    ...
    rs->error_roots = malloc( 2 * rs->min_distance * sizeof(field_element_t));
    ...
    for (uint16_t i = 0; i < rs->min_distance; i++) {
...
ssize_t correct_reed_solomon_decode(correct_reed_solomon *rs, const uint8_t *encoded, size_t encoded_length,
    ...
    for (uint16_t i = 0; i < pad_length; i++) {
    ...
    uint16_t order = reed_solomon_find_error_locator(rs, 0);
    // XXX fix this vvvv
...
ssize_t correct_reed_solomon_decode_with_erasures(correct_reed_solomon *rs, const uint8_t *encoded,
    ...
    for (uint16_t i = 0; i < erasure_length; i++) {
    ...
    for (uint16_t i = erasure_length; i < rs->min_distance; i++) {
        rs->syndromes[i - erasure_length] = rs->modified_syndromes[i];
    }

    uint16_t order = reed_solomon_find_error_locator(rs, erasure_length);
    // XXX fix this vvvv
    rs->error_locator.order = order;
