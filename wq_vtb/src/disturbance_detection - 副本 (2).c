    ...
    int32_t expect_pos, float ave_mag, wq_prm_bit_val_t *preamble_bits_data,int32_t *sample_data)
{
    ...
    if(expect_pos < 0){ //if there is no expect position, certain possible positions need to return
        if(temp_num < 10){ //if not find any possible position, return directly
            preamble_bits_data->bit_cnt = 0;
            return 0;
        }
        ...
        if(group_num <= PREAMBLE_BIT_GROUP){
            ...
            wq_dbg_printf("wave len %d, average magnitude %f, current group max magnitude %f.", find_sample, ave_mag, max_mag_expect_pos);
            return ret;
        }
    ...
    uint8_t sample_order, int32_t exp_pos, float ave_mag, wq_prm_bit_val_t *preamble_bits_data)
{
    ...
    //int32_t ave_result1[data_len];
    //int32_t ave_result2[data_len];
    int32_t* ave_result1 = malloc(sizeof(int32_t) * data_len);
    ...
    mask = statis_decision(phase, wave_analysis, (data_len+1)/2, sample_order, exp_pos, ave_mag, preamble_bits_data,sample_data);
