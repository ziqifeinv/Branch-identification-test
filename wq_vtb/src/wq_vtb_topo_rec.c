...
uint8_t *wq_vtb_topo_rx_create(uint8_t pop_num)
{
    ...
    memset(ptr, 0, sizeof(rx_topo_init_t));
    ...
    uint8_t *timing_mag, float *timing_val, uint32_t buf_len, uint32_t scale)
{
    ...
    for (b_idx = 0; b_idx < PREAMBLE_BIT_LEN; b_idx++) {
        ...
        for (g_idx = 0; (g_idx < p_prm->bit_cnt) && (g_idx < PREAMBLE_BIT_GROUP); g_idx++) {
    ...
    uint32_t scale, uint32_t delta, wq_prm_val_t *preamble, uint32_t preamble_cnt)
{
    ...
    for (i = pos_cnt - 1; i > 0; i--) {
        if ((timing_pos[i] % scale) < (128 + 32)
            && (timing_pos[i - 1] % scale) > scale - (128 + 32)) {
            if ((timing_pos[i] - timing_pos[i - 1]) < (128 + 32)) {
                if (abs(timing_val[i] - timing_val[i - 1]) < 800) {
                    timing_pos[i] = (timing_pos[i] + timing_pos[i - 1]) / 2;
                        timing_pos[i - 1] = timing_pos[i];
                }
            }
        }
    }
    ...
    while (cur_pos >= end_pos) {
        ...
        for (bit = PREAMBLE_LENGTH; bit > 0; bit--) {
            ...
            for (i = pos_cnt - 1; ((i >= 0) && (timing_pos[i] >= pre_pos)); i--) {
                if ((timing_pos[i] > pre_pos) && (timing_pos[i] < pst_pos)) {
                    preamble[preamble_idx].bits_pos[bit - 1] = timing_pos[i] % scale;
...
{
    unsigned long *preamble_vector = preuso_noise_sequence_topo(), cur_preamble = 0;
...
0 means not found.
*/
uint32_t wq_preamble_detect(rx_topo_init_t *rx, uint32_t phase, uint32_t scale,
    uint32_t delta, wq_prm_val_t *preamble_data)
{
    wq_prm_val_t preamble[PREAMBLE_BIT_GROUP * PREAMBLE_LENGTH], *final_preamble = NULL;
    uint32_t preamble_cnt, frame_len, ret = 0, lost_bits, pos_bits = 0, neg_bits = 0;
    ...
    if (NULL != final_preamble) {
        if (lost_bits <= rx->popcnt_threshold && (WQ_VTB_AMP_ERR_BITS >= neg_bits)) {
            ...
            wq_info_printf("phase %d preamble not found, %08x, "
                "lost bits %d, pos %d, frm_len %d, pos_bits %d, neg_bits %d.",
                phase, final_preamble->preamble, lost_bits, final_preamble->pos,
                frame_len, pos_bits, neg_bits);
        }
    } else {
        wq_info_printf("phase %d preamble not found.", phase);
    }
    ...
    wq_prm_bit_val_t *preamble_bit)
{
    ...
    wq_dbg_printf("phase %d bit, cout %d, pos:%d,%d,%d, sign:%d,%d,%d, val:%d,%d,%d",
        phase, preamble_bit->bit_cnt,
    ...
    wq_prm_bit_val_t *p_prm_bit)
{
    ...
    //if find preamble, calculate the message length, and reset related state variables
    if (frame_length > 0) {
...
uint32_t wq_preamble_copy_2(iot_bits_timming_t* timing, wq_prm_bit_val_t* bits)
{
    uint32_t g_idx, b_idx, bits_cnt = 0, detla_scale;
...
uint32_t wq_preamble_scan_rough_2(uint32_t phase, iot_bits_timming_t* timming,
    uint32_t bits_cnt, uint32_t prmb_pttn, iot_rx_topo_preamble_t* preamble)
{
    uint32_t pos_cur = 0, pos_end = 0, pos_pre = 0, pos_pst = 0, cur_bit = 0;
    uint32_t preamble_found = 0, bits_error = 0;
    uint16_t bits_active = 0, bits_negative = 0, bits_act_old = 0, bits_neg_old = 0;
    ...
    for (i = 0; i <= bits_cnt - 1; i++) {
        temp_pos[i] = timming[i].pos % PREAMBLE_SCALE;
        temp_sign[i] = timming[i].sign;
    }
    /* when signal are separated by two data sets, they need to be merged */
    for (i = bits_cnt - 1; i > 0; i--) {
        if ((timming[i].pos % PREAMBLE_SCALE) < (128 + 32)
            && (timming[i - 1].pos % PREAMBLE_SCALE) > PREAMBLE_SCALE - (128 + 32)) {
            if ((timming[i].pos - timming[i - 1].pos) < (128 + 32)) {
                if (abs(timming[i].val - timming[i - 1].val) < 800) {
            ...
            if (cur_bit == ((prmb_pttn >> bit_idx) & 0x1)) {
                if (cur_bit) {
                    ...
                    if (pos_last == 0) {
                        ...
                        pos_next = ((timming[tm_idx].pos % PREAMBLE_SCALE) * WQ_VTB_POS_WERGHT
                            + (pos_last % PREAMBLE_SCALE) * (10 - WQ_VTB_POS_WERGHT)) / 10;
                        pos_next += (timming[tm_idx].pos / PREAMBLE_SCALE) * PREAMBLE_SCALE;
...
//        if ((bits_negative <= WQ_VTB_AMP_ERR_BITS + merged_counter)
            ...
            if (bit1_cnt > 0) {
                ...
                for (i = 0; i < PREAMBLE_LENGTH; i++) {
                    if ((preamble_found >> i) & 0x1) {
                        if (abs(timming[bit1_pos_save[i]].val * 3) <= temp_ave_val) {
                            preamble_found &= ~(1 << i);
                        }
                    }
                }
    ...
    memset(preamble, 0, sizeof(iot_rx_topo_preamble_t));
    memcpy(preamble, &almost_prmb, sizeof(almost_prmb));

    wq_info_printf("phase:%d, almost preamble %08x, lost bits %d, pos %d, "
        "val %d, pos_bits %d, neg_bits %d.",
        phase, preamble->prmb, preamble->err_bits, preamble->pos,
        (int)preamble->val, bits_act_old, bits_neg_old);
    ...
    iot_rx_topo_preamble_t* preamble)
{
    ...
    if (data_len > 0) {
        wq_info_printf("phase %d preamble found, %08x, lost bits %d, pos %d, ave_val:%d, frm_len %d.",
            phase, preamble->prmb, preamble->err_bits, preamble->pos,
            (int)preamble->val, data_len);
    }
    else {
        wq_info_printf("phase %d preamble not found, %08x, lost bits %d, pos %d, ave_val:%d, frm_len %d.",
            phase, preamble->prmb, preamble->err_bits, preamble->pos,
            (int)preamble->val, data_len);
    }
...
uint8_t decode_data_per_phase(rx_topo_init_t *rx,  uint8_t phase, uint8_t mask,
    uint8_t *decoded_data, uint8_t *decoded_data_len, uint8_t *reset)
{
    uint8_t flag = 0;
    ...
    rx->msg_count[phase]++;
    if(rx->packet_det_flag[phase] && rx->msg_count[phase]
        <= (rx->rec_msg_len[phase] * 8)){
    ...
    wq_dbg_printf("phase %d frame complate, msg_count=%d.", phase, rx->msg_count[phase]);
...
uint8_t wq_vtb_topo_bit_rec(uint8_t *rx_ptr,  uint8_t phase, int32_t *sample_data,
    uint16_t data_len, uint8_t sample_order, uint8_t *decoded_data,
    uint8_t *decoded_data_len, int8_t *offset, int16_t *data_offset){
...
*/
    ...
    //demodulate sample data based on wavelet analysis. the results are stored in mask.
    uint8_t mask = disturb_detection(phase, sample_data, data_len,
        sample_order, pos, rx->avg_value[phase], &preamble_bit);

    //if (phase == 0) {
    //    wq_dbg_printf("mask:%d, pos:%d, correct_offset:%d.",mask,pos,rx->correct_offset[phase]);
    //    wq_vtb_adc_data_format_print(sample_data,data_len,phase);
    //}
    *offset = 0;
    ...
    *data_offset = 10;
    ...
    if (reset) {
        ...
        rx->conv = correct_convolutional_create(2, 7, correct_conv_r12_7_polynomial);
    }

    wq_dbg_printf("phase %d decode_flag=%d, *decoded_data_len=%d, *offset=%d, *data_offset=%d.",
        phase, decode_flag, *decoded_data_len, *offset, *data_offset);
