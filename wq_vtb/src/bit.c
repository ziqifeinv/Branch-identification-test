...
void bit_writer_write(bit_writer_t *w, uint8_t val, uint16_t n) {
...
bit_reader_t *bit_reader_create(const uint8_t *bytes, size_t len) {
    bit_reader_t *r = malloc( sizeof(bit_reader_t));
...
uint8_t bit_reader_read(bit_reader_t *r, uint16_t n) {
    uint16_t read = 0;
    uint16_t n_copy = n;
    ...
    //copy_mask <<= (8 - r->current_byte_len); // LSB read first
