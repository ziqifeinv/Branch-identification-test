...
void polynomial_mul(field_t field, polynomial_t l, polynomial_t r, polynomial_t res) {
    ...
    for (uint16_t i = 0; i <= l.order; i++) {
        if (i > res.order) {
            continue;
        }
        uint16_t j_limit = (r.order > res.order - i) ? res.order - i : r.order;
        for (uint16_t j = 0; j <= j_limit; j++) {
...
void polynomial_mod(field_t field, polynomial_t dividend, polynomial_t divisor, polynomial_t mod) {
    ...
    for (uint16_t i = dividend.order; i > 0; i--) {
        ...
        uint16_t q_order = i - divisor.order;
        ...
        for (uint16_t j = 0; j <= divisor.order; j++) {
...
void polynomial_formal_derivative(field_t field, polynomial_t poly, polynomial_t der) {
    ...
    for (uint16_t i = 0; i <= der.order; i++) {
...
field_element_t polynomial_eval(field_t field, polynomial_t poly, field_element_t val) {
    ...
    for (uint16_t i = 0; i <= poly.order; i++) {
        if (poly.coeff[i] != 0) {
...
field_element_t polynomial_eval_log_lut(field_t field, polynomial_t poly_log, const field_logarithm_t *val_exp) {
    ...
    for (uint16_t i = 0; i <= poly_log.order; i++) {
...
polynomial_t polynomial_init_from_roots(field_t field, uint16_t nroots, field_element_t *roots, polynomial_t poly, polynomial_t *scratch) {
    ...
    for (uint16_t i = 1; i < nroots; i++) {
        l.coeff[0] = roots[i];
        uint16_t nextrcoeff = rcoeffres;
...
polynomial_t polynomial_create_from_roots(field_t field, uint16_t nroots, field_element_t *roots) {
    ...
    polynomial_t l;
    l.order = 1;
    l.coeff = malloc( 2 * sizeof(field_element_t));

    polynomial_t r[2];
    // we'll keep two temporary stores of rightside polynomial
    // each time through the loop, we take the previous result and use it as new rightside
    // swap back and forth (prevents the need for a copy)
    r[0].coeff = malloc( (order + 1) * sizeof(field_element_t));
    r[1].coeff = malloc( (order + 1) * sizeof(field_element_t));
    uint16_t rcoeffres = 0;
