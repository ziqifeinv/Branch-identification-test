...
/*void conv_decode_print_iter(correct_convolutional *conv, uint16_t iter,
    ...
    for (shift_register_t i = 0; i < conv->numstates / 2; i++) {
        printf("%2d: ", i);
        for (uint16_t j = 0; j <= winner_index; j++) {
...
void convolutional_decode_warmup(correct_convolutional *conv, uint16_t sets,
                                 const uint8_t *soft) {
    ...
    for (uint16_t i = 0; i < conv->order - 1 && i < sets; i++) {
...
void convolutional_decode_inner(correct_convolutional *conv, uint16_t sets,
                                const uint8_t *soft) {
    shift_register_t highbit = 1 << (conv->order - 1);
    for (uint16_t i = conv->order - 1; i < (sets - conv->order + 1); i++) {
        ...
        if (soft) {
            if (conv->soft_measurement == CORRECT_SOFT_LINEAR) {
                for (uint16_t j = 0; j < 1 << (conv->rate); j++) {
                    distances[j] =
            ...
            uint16_t out = bit_reader_read(conv->bit_reader, conv->rate);
            for (uint16_t i = 0; i < 1 << (conv->rate); i++) {
...
void convolutional_decode_tail(correct_convolutional *conv, uint16_t sets,
                               const uint8_t *soft) {
    // flush state registers
    // now we only shift in 0s, skipping 1-successors
    shift_register_t highbit = 1 << (conv->order - 1);
    for (uint16_t i = sets - conv->order + 1; i < sets; i++) {
        // lasterrors are the aggregate bit errors for the states of shiftregister for the previous
        // time slice
        const distance_t *read_errors = conv->errors->read_errors;
        // aggregate bit errors for this time slice
        ...
        const uint16_t *table = conv->table;

        // a mask to get the high order bit from the shift register
        uint16_t num_iter = highbit << 1;
        uint16_t skip = 1 << (conv->order - (sets - i));
        uint16_t base_skip = skip >> 1;

        shift_register_t highbase = highbit >> 1;
        for (shift_register_t low = 0, high = highbit, base = 0; high < num_iter;
             low += skip, high += skip, base += base_skip) {
            uint16_t low_output = table[low];
            uint16_t high_output = table[high];
...
void _convolutional_decode_init(correct_convolutional *conv, uint16_t min_traceback,
                                uint16_t traceback_length, uint16_t renormalize_interval) {
    conv->has_init_decode = true;

    conv->distances = malloc( 1 << (conv->rate) * sizeof(distance_t));
...
static ssize_t _convolutional_decode(correct_convolutional *conv, size_t num_encoded_bits,
    ...
    if (!conv->has_init_decode) {
        ...
        uint16_t renormalize_interval = distance_max / max_error_per_input;
