...
static polynomial_t reed_solomon_build_generator(field_t field, uint16_t nroots, field_element_t first_consecutive_root, uint16_t root_gap, polynomial_t generator, field_element_t *roots) {
...
correct_reed_solomon *correct_reed_solomon_create(field_operation_t primitive_polynomial, field_logarithm_t first_consecutive_root, field_logarithm_t generator_root_gap, size_t num_roots) {
    correct_reed_solomon *rs = malloc( sizeof(correct_reed_solomon));
...
/* void correct_reed_solomon_debug_print(correct_reed_solomon *rs) {
    ...
    printf("generator: ");
    for (uint16_t i = 0; i < rs->generator.order + 1; i++) {
    ...
    for (uint16_t i = rs->generator.order + 1; i > 0; i--) {
    ...
    for (uint16_t i = 0; i < rs->encoded_remainder.order + 1; i++) {
    ...
    has_printed = false;
    for (uint16_t i = 0; i < rs->error_locator.order + 1; i++) {
    ...
    has_printed = false;
    for (uint16_t i = 0; i < rs->error_evaluator.order; i++) {
    ...
    has_printed = false;
    for (uint16_t i = 0; i < rs->error_locator_derivative.order; i++) {
